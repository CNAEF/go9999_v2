...
(function (w, d, $) {
    ...
    $('#form_type').change(function(){
        if ($(this).val() == 2) {
            $('#primary_school_type_group').show();
        } else {
            $('#primary_school_type_group').hide();
        }
    });

    // 公益团队支持 -> 团队名称, 开展项目
    $(':radio[name=is_supported]').change(function(){
        if ($(this).val() == 1) {
            $('#public_name_group').show();
            $('#project_group').show();
        } else {
            $('#public_name_group').hide();
            $('#project_group').hide();
        }
    });

    // 需要志愿者 -> 需要数量, 工作内容
    $(':radio[name=need_volunteer]').change(function(){
        if ($(this).val() == 1) {
            $('#need_volunteer_count_group').show();
            $('#volunteer_work_group').show();
        } else {
            $('#need_volunteer_count_group').hide();
            $('#volunteer_work_group').hide();
        }
    });

    // 其他需要 -> 列举其他需要
    $(':radio[name=need_other]').change(function(){
        if ($(this).val() == 1) {
            $('#need_other_content_group').show();
        } else {
            $('#need_other_content_group').hide();
        }
    });
